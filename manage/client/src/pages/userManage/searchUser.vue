...
export default {
  ...
  methods: {
    ...mapActions(["searchUserAction", "resetListAction", "statusAction","getUserListAction"]),
    handleUserSearch() {
      const value = this.value;
      const select = this.select === "手机号" ? 1 : 2;
      if (select === 1) {
        if (value - 0) {
          this.searchUserAction({ type: select, keyword: value, that: this });
        } else {
          this.$message({
            message: "请填写数字！",
            type: "warning"
