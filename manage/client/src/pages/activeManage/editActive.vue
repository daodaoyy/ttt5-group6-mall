  ...
  <div>
    <div class="active">
      ...
      <div class="active-main">
        <p 
          style="margin-left:-85%;font-size:14px;"
        >
          全部商品:
        </p>
        <div class="data1">
      ...
      >
        <div 
          v-if="img.data_cover == '' ? false : true"
          class="Img"
        >
          <li 
            @click="delImg1"
        ...
        <div
          v-if="img.data_detail == '' ? false : true"
          class="Img1"
        >
          <li
            @click="delImg2"
...
export default {
    ...
    methods:{
      ...
      'editActiveInfoAction',//创建活动
      'getTokenAction',//获取token
      'searchGoodsAction'
      ]),
       getActiveInfo(){
         let Info = this.$store.state.active.active_detail
         if(Info.productList !== undefined){
           Info.productList.map(item => {
             this.goods_id.push(item.id)
             this.discount.push(item.discount)
             this.img.data_cover = Info.aInfo.cover
             this.img.data_detail = Info.aInfo.detail
             this.ActiveId = Info.aInfo.id
           })
           this.tableData1 = Info.productList
         }
       },
       //删除已有图片
      delImg1() {
  ...
  //  上传详情图
    ...
    //提交编辑活动
    onSubmit(){
      if((this.img.detail == '' && this.img.data_detail == '') || (this.img.cover == '' && this.img.data_cover == '')){
        this.$message({
          message: '活动相关图片不能为空',
          type: 'warn'
        })
        ...
        let obj = {
          data:{
            ...
            cover: this.img.cover !== '' ? this.img.cover : this.img.data_cover,
            detail: this.img.detail !== '' ? this.img.detail : this.img.data_detail
          },
          that: this,
          id:this.ActiveId
        }
        this.editActiveInfoAction(obj)
      }
    },
    handleClose(done) {
